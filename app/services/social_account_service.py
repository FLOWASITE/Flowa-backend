...
class SocialAccountService:
    ...
    async def create_social_account(self, account_data: SocialAccountCreate):
        """Tạo mới tài khoản mạng xã hội"""
        conn, cursor = self._get_connection_and_cursor()

        try:
            print(f"Đang tạo tài khoản cho: {account_data}")

            # Truy vấn platform_id từ platform_name
            print(f"Truy vấn platform_id cho platform_name: {account_data.platform_name}")
            ...
            print(f"platform_id được tìm thấy: {platform_id}")
            ...
            if not row:
                raise HTTPException(status_code=500, detail="Tạo tài khoản mạng xã hội thất bại.")
    ...
    async def update_social_account(self, account_id: str, account_data: SocialAccountUpdate):
        ...
        try:
            ...
            cursor.execute(
                f"UPDATE social_accounts SET {set_clause} WHERE account_id = %s",
                tuple(values)
            )
            conn.commit()
    ...
    async def delete_social_account(self, account_id: str):
        """Xoá tài khoản mạng xã hội"""
        conn, cursor = self._get_connection_and_cursor()

        try:
            cursor.execute("SELECT id FROM social_accounts WHERE account_id = %s", (account_id,))
            if not cursor.fetchone():
                raise HTTPException(status_code=404, detail="Không tìm thấy tài khoản để xoá.")

            cursor.execute("DELETE FROM social_accounts WHERE account_id = %s", (account_id,))
            conn.commit()
